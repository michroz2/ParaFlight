...
## [1.22.1] - 2026-09-13
### Fixed
- Устранение утечки памяти в анимации карты (AnimationController вынесен на уровень класса с корректным dispose).
- Защита файловой системы от крашей (обрабатывается отказ в выдаче прав на запись и ошибки в saveTrack).
### Changed
- Оптимизация производительности в WindCirclePainter (вынесены объекты Paint для снижения нагрузки на GC).
- Очистка логов (удален тестовый таймер из main.dart).

## [1.22.0] - 2026-09-12
### Added
- Внедрен режим Advanced Mode (скрытые настройки) для опытных пользователей (активируется 7-кратным нажатием на версию).
- Экран "О приложении" с функцией Factory Reset.

### Changed
- Обновлены и адаптированы тексты в настройках.
- Скорректированы дефолтные значения конфигурации для модуля ветра, параметров трека и режимов экрана.
- Сброс расширенного режима при переключении на встроенный GPS.

## [1.21.6] - 2026-09-12
### Changed
- Улучшение UI:
  - Удалены единицы измерения ('км', 'м/с') из числовых индикаторов на компасе и карте, согласно требованию.
  - Внесены правки в `WindCirclePainter.dart`.
  - изменена иконка приложения на PF5.png

## [1.21.5] - 2026-09-12
### Changed
- Динамическое центрирование UI: смещение карты и компаса отключено (установлено в 0) в режиме симуляции для предотвращения перекрытия с панелью плеера.

## [1.21.4] - 2026-09-12
### Fixed
- Исправлен баг рассинхронизации вращения карты. Устранен локальный стейт _rotationMode, управление переведено на единый mapSettingsProvider.

## [1.21.3] - 2026-09-12
### Added
- Добавлены аппаратные ограничители зума карты (minZoom: 8.5, maxZoom: 18.5) для соответствия массиву standardRadii.

## [1.21.2] - 2026-09-12
### Fixed
- Исправлено направление стрелки указателя на старт (повернута на 180 градусов, теперь указывает от пилота к цели).

## [1.21.1] - 2026-09-12
### Fixed
- Исправлено округление дистанции до точки старта (теперь отображаются десятые доли километра вместо целых чисел).

## [1.21.0] - 2026-09-12
### Added
- Добавлен зеленый указатель направления на точку старта (Return-to-Launch) с настройкой видимости поверх компаса ветра.

## [1.20.13] - 2026-09-12
### Added
- Внедрен декартов EMA-фильтр (`windEmaAlpha`) для стрелки компаса ветра. Настройки инерции добавлены в UI.

## [1.20.12] - 2026-09-12
### Changed
- Исправление логики маршрутизации потоков ветра: при отсутствии маневров (полет по прямой) старая валидная стрелка компаса больше не становится серой, а сохраняет валидность (согласно CQRS архитектуре).

## [1.20.11] - 2026-09-11
### Changed
- Разделение потоков ветра. Телеметрия отображает актуальные забракованные данные, компас сохраняет последнюю валидную стрелку (серую).
- Внедрено ополовинивание буфера (аэродинамическая промывка) вместо полной очистки при отбраковке данных.

## [1.20.10] - 2026-09-11
### Changed
- Улучшение UI: Цветовая индикация (красный цвет) для параметров ветра (Airspeed, RMSE, Roundness), выходящих за рамки настроек валидации в окне телеметрии.

## [1.20.9] - 2026-09-11
### Changed
- Улучшение UI панели симулятора: иконка Replay и логика автоматического перезапуска трека при окончании симуляции.

## [1.20.8] - 2026-09-11
### Added
- Настройки валидации ветра (Roundness, RMSE, Buffer Size) вынесены в пользовательский интерфейс.
- Внедрена умная защита буфера ветра от истощения по таймеру (сохраняется минимально необходимое число точек для работы алгоритма).

## [1.20.7] - 2026-09-11
### Changed
- Рефакторинг (DRY). Унификация логики перемотки симулятора.
- В `PlaybackState` добавлено единое поле-счетчик `seekCount`.
- Из `WindNotifier` и `FlightDetectorNotifier` удалены костыльные проверки индексов и таймингов для обнаружения прыжков во времени.
- Модули ветра и детектора полета теперь реактивно слушают централизованный сигнал `seekCount` и гарантированно сбрасывают свои буферы при перемотке.

## [1.20.5] - 2026-09-11
### Changed
- Архитектурный рефакторинг конвейера ветра (Separation of Concerns).
- Внутренняя валидация перенесена из математического ядра в конвейер (WindPipeline).
- Поле `isStale` переименовано в `isValid`.
- Забракованные данные больше не уничтожаются бесследно, а пробрасываются в UI с флагом `isValid: false`. 
- В UI компас становится серым при `!isValid`, но параметры (RMSE, Roundness) продолжают отображаться в телеметрии.
