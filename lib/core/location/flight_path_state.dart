...
final flightPathProvider = Provider<List<TrackSegment>>((ref) {
  ...
  if (dataSource == DataSource.internalGps) {
    rawPoints = ref.watch(realGpsTrackProvider);
  } else {
    // Логика симулятора
    final gpxStateAsync = ref.watch(gpxPointsProvider);
    final gpxState = gpxStateAsync.valueOrNull;
    
    if (gpxState != null && gpxState.points != null && gpxState.points!.isNotEmpty) {
      final currentIndex = ref.watch(playbackProvider.select((s) => s.currentIndex));
      rawPoints = gpxState.points!.sublist(0, currentIndex + 1).map((e) => LatLng(e.latitude, e.longitude)).toList();
    }
  }
  ...
  for (final flight in detectorState.flights) {
    ...
    int startIndex = rawPoints.indexWhere((p) => p.latitude == startLoc.latitude && p.longitude == startLoc.longitude, currentIndexInRaw);
    
    if (startIndex == -1) continue;

    // Добавляем наземный сегмент ДО этого старта
    if (startIndex > currentIndexInRaw) {
      segments.add(TrackSegment(points: rawPoints.sublist(currentIndexInRaw, startIndex + 1), isFlight: false));
    }

    if (flight.finish == null) {
      // Полет еще не закончен
      segments.add(TrackSegment(points: rawPoints.sublist(startIndex), isFlight: true));
      currentIndexInRaw = rawPoints.length;
      break;
    }

    final finishLoc = LatLng(flight.finish!.latitude, flight.finish!.longitude);
    int finishIndex = rawPoints.indexWhere((p) => p.latitude == finishLoc.latitude && p.longitude == finishLoc.longitude, startIndex);
    ...
    segments.add(TrackSegment(points: rawPoints.sublist(startIndex, finishIndex + 1), isFlight: true));
  ...
  if (currentIndexInRaw < rawPoints.length - 1) {
    segments.add(TrackSegment(points: rawPoints.sublist(currentIndexInRaw), isFlight: false));
  }
