...
void main() {
  final file = File('lib/features/flight_detector/application/flight_detector_pipeline.dart');
  var content = file.readAsStringSync();

  content = content.replaceAll('  final List<FlightRecord> _flights = [];', '''  final List<FlightRecord> _flights = [];
  
  double _totalTrackDistance = 0.0;
  Duration _totalTrackDuration = Duration.zero;
  LocationEntity? _lastLocation;''');

  content = content.replaceAll('    _currentState = FlightState.groundMovement;', '''    _currentState = FlightState.groundMovement;
    _totalTrackDistance = 0.0;
    _totalTrackDuration = Duration.zero;
    _lastLocation = null;''');

  content = content.replaceAll('  List<LocationEntity> get buffer => _buffer;', '''  List<LocationEntity> get buffer => _buffer;
  ...
  content = content.replaceAll("import '../domain/flight_state.dart';", "import 'package:latlong2/latlong.dart' as latlong2;\nimport '../domain/flight_state.dart';");

  var old_process = '''  void processLocation(LocationEntity point, {WindCalculationResult? currentWind}) {
    _buffer.add(point);
    
    // Поддерживаем буфер размером 60 секунд''';
  var new_process = '''  void processLocation(LocationEntity point, {WindCalculationResult? currentWind}) {
    ...
    // Поддерживаем буфер размером 60 секунд''';
  content = content.replaceAll(old_process, new_process);

  content = content.replaceAll('    } // конец if-else\n  } // конец метода processLocation', '    } // конец if-else\n    _lastLocation = point;\n  } // конец метода processLocation');

  var takeoff_old = '''    // Паттерн совпал!
    _flights.add(FlightRecord(start: p1));
    _currentState = FlightState.inFlight;''';
  var takeoff_new = '''    // Паттерн совпал!
    ...
    _currentState = FlightState.inFlight;''';
  content = content.replaceAll(takeoff_old, takeoff_new);

  var midair_old = '''      if (isHighSpeed) {
        final start = recentPoints.first;
        _flights.add(FlightRecord(start: start, isMidAirStart: true));
        _currentState = FlightState.inFlight;''';
  var midair_new = '''      if (isHighSpeed) {
        final start = recentPoints.first;
        ...
        _currentState = FlightState.inFlight;''';
  content = content.replaceAll(midair_old, midair_new);

  var landing_old = '''    // Паттерн совпал!
    if (_flights.isNotEmpty) {
      _flights.last.finish = p2;
    }
    _currentState = FlightState.landing;''';
  var landing_new = '''    // Паттерн совпал!
    if (_flights.isNotEmpty) {
      _flights.last.finish = p2;
    ...
    _currentState = FlightState.landing;''';
  content = content.replaceAll(landing_old, landing_new);
